...
def CPA_find_key_byte(bytenum: int) -> int:
	...
	for line in range(len(V)):
		for column in range(len(V[0, :])):
			H[line, column] = HW[V[line, column]]
	...
	for key_index in range(len(K)):
		...
		for trace_index in range(trace_length):
			R[key_index, trace_index] = np.corrcoef(H[:, key_index], datapoints[:, trace_index])[0, 1] 
	...
	# #bytenum.
	key_found = np.unravel_index(np.argmax(abs(R), axis=None), R.shape)[0]
# TODO (you can use abs(R).argmax())
